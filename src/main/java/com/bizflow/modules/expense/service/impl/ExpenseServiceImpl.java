...
public class ExpenseServiceImpl implements ExpenseService {
        ...
        public ApiResponse<List<ExpenseDto>> getAll() {
                ...
                return ApiResponse.success(
                                expenseRepository.findAllByTenantIdOrderByExpenseDateDesc(tenantId).stream()
        ...
        public ApiResponse<List<ExpenseDto>> getByDateRange(LocalDate from, LocalDate to) {
                Long tenantId = SecurityUtils.getCurrentTenantId();
                return ApiResponse.success(
                                expenseRepository.findAllByTenantIdAndExpenseDateBetween(tenantId, from, to).stream()
                                                .map(this::toDto).toList());
        }
        ...
        public ApiResponse<ExpenseDto> create(ExpenseDto dto) {
                ...
                ExpenseCategory category = dto.getCategoryId() != null
                                ? categoryRepository.findByIdAndTenantId(dto.getCategoryId(), tenantId).orElse(null)
                                : null;
