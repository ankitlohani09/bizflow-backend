...
public class AuthServiceImpl implements AuthService {
    ...
    public ApiResponse<LoginResponse> login(LoginRequest request) {
        ...
        String token = jwtService.generateToken(user.getEmail(), user.getId(), user.getTenantId(), roles);

        LoginResponse response = LoginResponse.builder().token(token).tokenType("Bearer").userId(user.getId())
                .tenantId(user.getTenantId()).name(user.getName()).email(user.getEmail()).roles(roles).build();
